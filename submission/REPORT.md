...
Sau lượt đánh giá chính, tôi chạy lại đúng base model với `OPTIMIZED_PROMPT` đã đóng băng để lưu prediction-level vào `results/baseline_b_predictions.json`. Phép đối chiếu trên đủ 50 mẫu cho thấy fine-tune thắng baseline (b) ở 33 mẫu, hòa ở 17 mẫu và không thua ở mẫu nào. Vì vậy, tôi không thể đưa ra hai ca fine-tune thua (b) mà không làm sai lệch kết quả. Bảng dưới đây trình bày hai ca fine-tune thắng rõ ràng và ba ca fine-tune vẫn sai so với nhãn thật; ở ba ca sai này, (b) và (c) hòa nhau ở mức 3/4 trường.

| #   | Ticket (rút gọn)                                     | Nhãn đúng                                             | (b) prompt                 | (c) fine-tune                         | Nhận xét          |
| --- | ---------------------------------------------------- | ----------------------------------------------------- | -------------------------- | ------------------------------------- | ----------------- |
| 1   | Trả chuột không dây VN232232, gấp, shop hỗ trợ tốt   | `doi_tra, cao, chuột không dây, tich_cuc`             | Sai intent: `hoan_tien`, đạt 3/4 | Khớp nhãn 4/4 | ✅ FT thắng (b) |
| 2   | Nồi chiên DH249548 thiếu phụ kiện, khi nào tiện      | `san_pham_loi, thap, nồi chiên không dầu, trung_tinh` | Sai intent và urgency, đạt 2/4 | Chỉ sai urgency, đạt 3/4 | ✅ FT thắng (b) |
| 3   | Bình giữ nhiệt VN804124 chưa thấy tiền, khi nào tiện | `hoan_tien, thap, bình giữ nhiệt, tich_cuc`           | Sai urgency, đạt 3/4 | Sai urgency, đạt 3/4 | ❌ FT sai, hòa (b) |
| 4   | Áo khoác VN613097 bị lỗi, khi nào tiện, cảm ơn shop  | `san_pham_loi, thap, áo khoác gió, tich_cuc`          | Sai urgency, đạt 3/4 | Sai urgency, đạt 3/4 | ❌ FT sai, hòa (b) |
| 5   | Đèn bàn LED OD819229 sai màu, khi nào tiện           | `san_pham_loi, thap, đèn bàn LED, tich_cuc`           | Sai urgency, đạt 3/4 | Sai urgency, đạt 3/4 | ❌ FT sai, hòa (b) |

Mẫu lỗi chung ở cả ba trường hợp là cả baseline (b) lẫn fine-tune đều nâng `urgency` từ `thap` lên `trung_binh`, trong khi intent, product và sentiment vẫn đúng. Cụm “khi nào tiện” chưa tạo ra tín hiệu đủ mạnh để hai mô hình giữ mức khẩn cấp thấp khi ticket đồng thời đề cập hoàn tiền hoặc sản phẩm lỗi. Kết quả này gợi ý cần bổ sung các cách diễn đạt đa dạng cho urgency thấp và kiểm tra lại phân bố nhãn urgency. Quan trọng hơn, phép so sánh 33 thắng, 17 hòa, 0 thua cho thấy fine-tune cải thiện target một cách khá nhất quán; verdict FAILED không đến từ target mà đến từ mức suy giảm regression.

---

## 7. Kết luận và điều tôi học được

**Kết luận.** Tôi chưa nên deploy trực tiếp adapter này. Model đạt 0.97 target, giữ format ở mức 1.0 và, ở cấp độ từng mẫu, thắng baseline (b) 33 lần, hòa 17 lần, không thua lần nào. Những kết quả này chứng minh fine-tune đã học nhiệm vụ triage tốt và mức tăng target không chỉ đến từ một vài mẫu ngoại lệ. Tuy nhiên, regression giảm 0.147, vượt xa dung sai 0.02; kết quả phù hợp mạnh với hiện tượng catastrophic forgetting trên một corpus hẹp, dù thí nghiệm hiện tại chưa cô lập hoàn toàn mọi nguyên nhân có thể có. Nếu hệ thống chỉ làm triage trong một dịch vụ cô lập, adapter có thể được thử nghiệm giới hạn sau khi bổ sung giám sát, nhưng chưa đủ an toàn cho một trợ lý dùng chung. Thí nghiệm đối chứng cho thấy learning rate là đòn bẩy nhạy nhất: chỉ giảm từ 1e-4 xuống 1e-5 đã làm target về 0. Vị trí adapter và rank phải được xem trong cùng ngân sách; `attn_only` rank 283 hòa target với `correct` nhưng loss thấp hơn, chứng minh train loss không thể thay thế đánh giá tác vụ. Mask đã hoạt động đúng nên không phải nguyên nhân trực tiếp của regression. Bước tiếp theo là thêm replay data để duy trì năng lực chung, cân bằng thêm các ca urgency thấp, rồi đánh giá lại bằng đúng cổng đã đóng băng.
